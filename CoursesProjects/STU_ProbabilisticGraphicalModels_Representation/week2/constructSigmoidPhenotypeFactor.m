...
card1 = [];
for i=1:size(alleleWeights,2) % gene number
    card1 = [card1, size(alleleWeights{i},2)] ; % alles number in each gene
end
phenotypeFactor.card  = [2,repmat(card1,1,2)];

phenotypeFactor.val = zeros(1, prod(phenotypeFactor.card));
% Replace the zeros in phentoypeFactor.val with the correct values.
assignMat = IndexToAssignment(1:prod(phenotypeFactor.card),phenotypeFactor.card);
for i=1:size(assignMat,1)
    phen = assignMat(i,1);
    genes = [1:size(geneCopyVarOneList,1),1:size(geneCopyVarOneList,1)];
    alles = assignMat(i,2:end);
    suml = 0;
    for j=1:size(genes,2)
        suml = suml + alleleWeights{genes(j)}(alles(j));
    end
    if phen==1
        phenotypeFactor.val(i) = computeSigmoid(suml);
    else
        phenotypeFactor.val(i) = 1-computeSigmoid(suml);
    end
end
